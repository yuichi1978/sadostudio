                  ...
                  <dd class="p-project__search-content">
                    <select name="area" class="p-select">
                      ...
                      foreach ($custom_terms as $custom_term) {
                        ...
                        <option value="<?php echo $term_slug ?>" <?php if ($area === $term_slug) {
                                                                    echo 'selected';
                                                                  } ?>>
                ...
                <dl class="p-project__search-item">
                  ...
                  <dd class="p-project__search-content">
                    <select name="year" class="p-select">
                      ...
                      foreach ($custom_terms as $custom_term) {
                        $term_slug = urldecode($custom_term->slug); ?>
                        <option value="<?php echo $term_slug ?>" <?php if ($year === $term_slug) {
                                                                    echo 'selected';
                                                                  } ?>>
                          <?php echo $custom_term->name ?>
                        </option>
                ...
                <dl class="p-project__search-item">
                  ...
                  <dd class="p-project__search-content">
                    <select name="structure" class="p-select">
                      ...
                      foreach ($custom_terms as $custom_term) {
                        $term_slug = urldecode($custom_term->slug); ?>
                        <option value="<?php echo $term_slug  ?>" <?php if ($structure === $term_slug) {
                                                                    echo 'selected';
                                                                  } ?>>
                          <?php echo $custom_term->name ?>
                        </option>
                        ...
                        <li class="p-project__search-item">
                          <label class="p-project__search-label">
                            <input type="checkbox" class="checkbox-input" name="scale" value="<?php echo $term_slug; ?>"
                              <?php if (in_array($term_slug, $scale)) {
                                echo 'checked';
                              } ?>>
          ...
          <div class="p-section__inr c-width c-width--1200">
            <div class="l-work__list">
              <div class="l-work__list__inr">
                ...
                // URLパラメーターからエリアの値を格納したarea変数がallでなかった場合にarea_arrayに条件を入れていく
                if ($area != 'all') {
                  $area_array = array(
                    // この中で指定するのはタクソノミーとフィールドとタームズの３つになる。
                    // タクソノミーはどのタクソノミーで絞り込むのかフィールドはどの値で対象のタームで指定するのか
                    // タームズはどのタームに所属する投稿を引っ張ってくるのかの指定になる
                ...
                if (!empty($scale)) {
                  ...
                  $custom_terms = get_terms("projects_scale");
                  foreach ($custom_terms as $custom_term) {
                    $scale_slug = urldecode($custom_term->slug);
                    if (in_array($scale_slug, $scale)) {
                      array_push($scale_value, $scale_slug);
                    }
                  }
                  $scale_array = array(
                ...
                <?php } else { ?>
                  <p class="p-result__not">検索条件で該当する実績は0件です。検索条件を少なくし再検索をお願いいたします。</p>
                <?php };
                wp_reset_postdata();
                ?>
              </div>
